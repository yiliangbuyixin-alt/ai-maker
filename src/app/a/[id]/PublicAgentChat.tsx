...
export default function PublicAgentChat({ agentId }: { agentId: string }) {
  ...
  async function handleSend(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/a/${agentId}/chat`, {
        method: "POST",
  ...
  return (
    <div className="flex flex-col gap-2">
      <div
        ref={scrollRef}
        className="min-h-80 max-h-[60vh] overflow-y-auto flex flex-col gap-2 rounded-lg border border-black/10 dark:border-white/15 p-3"
      >
        {messages.length === 0 && (
